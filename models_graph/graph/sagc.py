...
#Creates multiple graph convolutions for different adjacency types:
#Static Adjacency (A) – Predefined structure from Graph.A
#Learnable Adjacency (g_adj) – Adjusted during training
#Attention-Based Adjacency (C) – Dynamically computed

class SAGC(nn.Module):
#Implementing Spatial Attention Graph Convolution (SAGC))
    """
    ...
    def __init__(self, in_channels, out_channels,
        ...
        for i in range(self.adj_types):  # 3 adjacency types (static, learned, attention)
        ...
        self.relu = nn.CELU(0.01)  

        self.kernel_size = self.stat_adj.size(0)  
    ...
    def forward(self, x, adj):
        ...
        x = self.reduction_conv(x)  # Reduce dimensionality
        y = x + x_residual   # Add residual connection
        return y, adj

    def calc_C(self, x):
        #The paper introduces a learned adjacency matrix C, computed dynamically for each input.
        N, chan, T, V = x.size()
