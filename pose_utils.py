...
def gen_clip_seg_data_np(clip_dict, start_ofst=0, seg_stride=4, seg_len=24, scene_id='', clip_id='', ret_keys=False,
    ...
    for idx in sorted(clip_dict.keys(), key=lambda x: int(float(x))):
        ...
        curr_pose_segs_np, curr_pose_segs_meta, curr_pose_score_np = split_pose_to_segments(sing_pose_np,
                                                                                            ...
                                                                                            dataset=dataset)
        if  dataset == 'NWPUC' or dataset=='Poselift':
...
def single_pose_dict2np(person_dict, idx):
    ...
    sing_pose_meta = [int(float(idx)), int(float(single_person_dict_keys[0]))]  
...
def split_pose_to_segments(single_pose_np, single_pose_meta, single_pose_keys, start_ofst=0, seg_dist=6, seg_len=12,
    ...
    single_pose_keys_sorted = sorted([int(float(i)) for i in single_pose_keys])  
